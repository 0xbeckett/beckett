...
over-engineers and burns minutes *and* the output is worse. A person judges these by eye, so the
right cast is **Opus @ `high` with `"reviewTier":"self"`** → one pass, no cold reviewer. Save pi
for things with a crisp spec and no pixels: APIs, parsers, data layers, scripts, migrations.

**On any frontend/UI ticket, invoke the [[ui-designer]] skill *before* you write the cast brief**
— it's the house aesthetic and the source-before-hand-roll workflow (check 21st.dev, then
shadcn/ui, then build). Bake it into the brief: name the skill, tell them to source a base
component before hand-rolling, and point them at its rubric for the self-review. (See the usage
note at the bottom of the skill for the one-paragraph brief template.)

If a ticket is genuinely mixed (backend + UI), prefer splitting it into two tickets so each gets
the right harness — a clean backend ticket (pi) and a clean frontend ticket (claude).
...
- **`pi` (gpt-5.6-terra, default; gpt-5.6-luna for the cheap lane)** — `medium` when the ticket
  body is really specific (sharp spec → medium is excellent and fast); `high` when it has to make
  real decisions; `xhigh` rare, crucial tasks only. Reach for an explicit
  `"model":"gpt-5.6-luna"` on cheap/mechanical low-effort grind.
- **`claude-opus-5`** — `high` for most tasks (the default choice), `xhigh` for the genuinely
  harder ones. Never below `high`.
...
- **`claude-fable-5`** — `high` as the standard (review or implement); `xhigh` only for the most
  crucial work, and remember every Fable cast was already confirmed with the human.

`xhigh` is rare across the whole fleet — reserved for crucial, hard-to-reverse work where a wrong
answer costs far more than the extra minutes. Casting it more than occasionally means you're
mis-sizing tickets.

**`effort` also picks the review gate (v3.1) — your main speed lever.** A worker self-reviews its
own diff against the criteria before finishing, so a second cold reviewer is often wasted relay
time. The dispatcher reads your cast `effort`:
...
- **`high`/`xhigh`, or omitted** → **fresh adversarial reviewer** runs after implement. Right for
...
turns, a visual toy that paid for a fresh reviewer are miscasts, and they're *your* miscasts.
...
work; the roster is a starting map, and this loop is how it gets corrected by reality.
...
- **`--channel` is how the loop closes — always pass it.** Every message you get is prefixed with
  a stamp like `[channel:<id>] [user:<userId> address:"…" msg:<messageId>]`; pass that same
  channel id as `--channel <id>` on `task create`. It creates the workspace and lets me ping the
  right conversation when the work hits review, ships, or breaks. Drop it and updates have
  nowhere to go.

After `task start`, give the human a one-liner using the public task reference, never the internal
ticket identifier: "Started #42 - Balloons physics; #42.1 is queued now." Keep the phrasing
honest: `task start` queues the work for pickup within seconds — "queued it" is true; "the tests
are running" may not be yet.
...
script, "add X to Y" — the main `#N.1` branch, started once, done. Add branches only when the
