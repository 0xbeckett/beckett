...
async function main(): Promise<void> {
  ...
  // ── mail (in-process: AgentMail SDK + ~/.beckett/mail.json durable inbox state) ─────────
  if (group === "mail") await runMail(sub, rest);
  ...
  fail(`unknown command: beckett ${group ?? ""} ${sub ?? ""}\n` +
    "commands: status [--pretty] | doctor [--json] | reload | persona | mail inbox|send|ls|read | access ls|grant|revoke | maintainer ls|grant|revoke | federation ls|add|remove | channels list|search|recall|wipe | identity set|show|list | discord reply|decline | proactivity status|set|off | quick <agent>|list | image | eval <author/model> [--short|--full] | site deploy | task create|branch|start|show|list | ticket create|comment|state|list|show|trace | preset ls|show | plan | gh repo|pr|push | dns ls|add|rm | deploy <name>|ls|rm | secret request | recall \"<query>\" [--type t] [--name n] [--as-self | --viewer id] | memory recall|remember|show|maintain");
}
