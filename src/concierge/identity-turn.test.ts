...
import { upsertPerson } from "../memory/people.ts";
...
test("a speaker's person file is loaded into the session once, and never someone else's", async () => {
  const dir = tmpBeckettDir();
  process.env.DISCORD_OWNER_ID = ALICE;
  const quiet = (() => {
  ...
  // Standing knowledge, not per-turn context: the same speaker is not re-introduced.
  await c.onMessage(message({ userId: ALICE, authorDisplayName: "alice", messageId: "m2" }));
  expect(stamp(asks[1]!)).not.toContain("<person");

  // A different speaker in the same room brings their own.
  await c.onMessage(message({ userId: BOB, authorDisplayName: "bob", messageId: "m3" }));
  expect(stamp(asks[2]!)).toContain(`<person user:${BOB}`);
  expect(stamp(asks[2]!)).toContain("bob only fact");
});

test("a speaker with no person file leaves the turn byte-identical", async () => {
  const dir = tmpBeckettDir();
  process.env.DISCORD_OWNER_ID = ALICE;
  const quiet = (() => {
    const log = { debug() {}, info() {}, warn() {}, error() {}, child: () => log };
